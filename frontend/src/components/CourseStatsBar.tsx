...
/** Compact holes / par / yards / rating strip under course identity. */
export function CourseStatsBar({ holes, par, yardage, rating }: Props) {
  const stats: Stat[] = [];
  ...
  if (typeof rating === 'number' && Number.isFinite(rating)) {
    stats.push({ label: 'Rating', value: rating.toFixed(1) });
  }
