-- Prefer today/tomorrow (hot) schedule rows when claiming poll work so Find
-- snapshots stay closer to live. Alerts still rank above non-alert peers within
-- the same day-offset tier.
...
begin
  ...
  with due as (
    ...
    order by
      (s.play_date - p_today_mt) asc,
      has_alert desc,
      s.last_polled_at nulls first,
      s.last_polled_at asc
    limit p_batch_size
...
comment on function public.claim_availability_poll_batch(date, date, integer, timestamptz) is
  'Claim due (course, date) poll rows. Hot dates (offset 0–1) first, then alert targets, then oldest last_polled_at. SKIP LOCKED for concurrent ticks.';
